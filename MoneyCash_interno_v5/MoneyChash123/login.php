    ...
    if ($email === '' || $senha === '') {
        ...
        try {
            ...
            if ($usuario && password_verify($senha, $usuario['senha'])) {
                ...
                $redirect = $_GET['redirect'] ?? 'painel.php';
    ...
    <main class="auth-layout">
        <section class="auth-brand-panel">
            ...
            <div class="auth-brand-content">
                <span class="auth-kicker">Gestão financeira</span>
                <h1>Seu dinheiro.<br><strong>Seu controle.</strong></h1>
                <p>Organize receitas, despesas, contas e metas em um único lugar.</p>
            </div>
        ...
        <section class="auth-form-panel">
            <div class="auth-card">
                ...
                <form method="POST" action="login.php<?= isset($_GET['redirect']) ? '?redirect=' . urlencode($_GET['redirect']) : '' ?>" class="auth-form">
                    <div class="form-grupo">
                        <label for="email">E-mail</label>
                        <input type="email" id="email" name="email" value="<?= htmlspecialchars($email) ?>" placeholder="voce@exemplo.com" autocomplete="email" required autofocus>
                    </div>
                ...
                <p class="auth-switch">Ainda não possui uma conta? <a href="cadastro.php">Criar conta</a></p>
            </div>
        </section>
    </main>
</body>
